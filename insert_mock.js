...
const sql = `INSERT INTO posts (id, type, sector, status, content, source_name, urgency, ai_score, created_at, updated_at) VALUES ('${id}', 'actualidad', 'laboral', 'draft', '${contentJson}', 'Diario AyE', 'alta', 9.8, datetime('now'), datetime('now'));`;

execSync(`npx wrangler d1 execute mytaxbot_linkedin --remote --command="${sql.replace(/"/g, '\\"')}"`, { stdio: 'inherit', cwd: 'c:\\Users\\elcho\\.gemini\\antigravity\\scratch\\LinkedIn\\workers' });
